...
// Definimos o estilo da fonte
        $objPHPExcel->getActiveSheet()->getStyle('A1')->getFont()->setBold(true)
					...
					->getActiveSheet()->getStyle('H1')->getFont()->setBold(true);
...
// Criamos as colunas
        if ($Nome == NULL){
            ...
            $objPHPExcel->setActiveSheetIndex(0)
                ...
                ->setCellValue("D1", "Cond. Negociais")
                ->setCellValue("E1", "Dt. Vcto.")
                ->setCellValue("F1", "Vlr. Dívida")
				->setCellValue("G1", "Class. Operação")
                ->setCellValue("H1", "Banco Nome");
        }
        ...
        while ($row = ibase_fetch_object($queryExp)) {
              $objPHPExcel
                ...
                ->setCellValue("D" . $linha, utf8_encode($row->ALUNO))
                ->setCellValue("E" . $linha, $fim = date('d/m/Y', strtotime($row->DATAVENC)))
                ->setCellValue("F" . $linha, $row->DIVIDA)
                ->setCellValue("G" . $linha, utf8_encode($row->CLASS_OPERACAO))
				->setCellValue("H" . $linha, utf8_encode($row->BANCO));
            $linha++;
        }
...
// Cabeçalho do arquivo para ele baixar
        ...
        header('Content-Disposition: attachment;filename= '.utf8_encode($Nome)." ".$data1." ".$data2.'.xlsx');
